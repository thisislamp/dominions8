...
func shoot_at_enemy():
	if closest_enemy and shoot_timer <= 0:
		...
		projectile_instance.rotation = $Marker2D.rotation * randf_range(.85, 1.15)
		projectile_instance.team_color = team_color
		projectile_instance.scale.x = .9
		projectile_instance.scale.y = .9
		projectile_instance.projectile_damage = 15
		projectile_instance.speed = 375
		projectile_instance.persistence_health = 10
